...
6. Evaluation: OOS Testing vs BTC Buy&Hold and S&P 500 benchmark (`yfinance`).
...
Algorithm: Stable-Baselines3 PPO with a Custom PyTorch Feature Extractor (CNN x2 -> LSTM x2).
Configurations: `ent_coef=0.01` (prevents LSTM highest-entropy collapse), minimum learning rate pinned at `1e-5` (prevents late-stage neural plasticity death), `n_steps=4096`, `batch_size=256` (16 gradient updates per BPTT batch), `max_grad_norm=0.5`.
</agent>
</core_components_state>
