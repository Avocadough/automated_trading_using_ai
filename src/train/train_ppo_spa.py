...
def main(
    ...
    common_env_kwargs = dict(
        ...
        taker_fee=0.001,         # 0.1% per side — เพิ่มให้ชัดเจน
        position_limit=0.50,
        slippage_bps=2.0,        # จำลอง slippage จริง
        reward_scale=100.0,
        normalize=True,
    ...
    model = PPO(
        ...
        n_steps=2048,           # เพิ่มขึ้น: สะสม experience มากขึ้นก่อน update
        batch_size=256,         # เล็กลง: gradient step ถี่ขึ้น
        n_epochs=10,            # update รอบต่อ rollout
        learning_rate=1e-4,     # ช้า: stable convergence
        gamma=0.99,
        gae_lambda=0.95,
        ent_coef=0.01,          # สำรวจมากขึ้น — สำคัญมากสำหรับ trading
        clip_range=0.20,
        vf_coef=0.5,
        max_grad_norm=0.5,
        policy_kwargs=dict(
...
if __name__ == "__main__":
    ...
    ap.add_argument("--features", type=str,
                    default="data/features/btc_15m_spa.parquet",
    ...
    ap.add_argument("--flat_penalty_bps", type=float, default=0.50)
    ap.add_argument("--inactivity_steps", type=int, default=64)
    ap.add_argument("--inactivity_penalty_bps", type=float, default=5.0)
